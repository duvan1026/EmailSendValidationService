...
namespace ServiceEmailSendValidation.Utilities
{
    public static class ImageUtils
    {
        ...
        /// </summary>
        /// <param name="imageBinary">Arreglo de bytes de la imagen.</param>
        /// <param name="imageFormat">El formato de la imagen (jpeg, png, etc.).</param>
        /// <returns>Etiqueta HTML <img> con la imagen en formato Base64.</returns>
        public static string ConvertImageToBase64Html(byte[] imageBinary, string imageFormat)
        {            
            string base64String = ConvertBytesToBase64(imageBinary);                             // Convierte el arreglo de bytes (la imagen) en una cadena Base64
            string htmlImage = $"<img src='data:image/{imageFormat};base64,{base64String}' />";  // Retorna la cadena en formato HTML img con el tipo de la imagen (jpeg, png, etc.)
